...
## ADR-193 — Twee weken premium op proef, één keer per apparaat
...
ADR-192 maakte premium van alles wat over weken gaat: voortgang, diploma's,
weekdoelen. Dat is precies wat een gezin zonder code nooit te zien krijgt. De
verkoop hing daarmee aan één zin ("Wil je zien wat je inmiddels kent?"), en een
kind dat gisteren nog typte of een tafeldiploma haalde, was dat de volgende dag
zonder uitleg kwijt.

### Besluit

**Elk apparaat krijgt één keer veertien dagen alles, en valt daarna terug naar
gratis.** Dat geldt voor een nieuw apparaat, en voor een apparaat dat er al was
bij de eerste keer openen na deze wijziging. Wat er in die dagen geoefend en
gehaald is, blijft staan; een diploma blijft een diploma.

- **Eén datum in localStorage** (`leernu.proef`), naast de code: de dag waarop
  de proef begon. Hij hoort bij het apparaat en niet bij een kind, en wordt op
  het eerste beeld gelezen, zodat er nooit eerst een slot staat dat daarna
  verdwijnt. Er gaat niets naar een server.
- **Alleen zonder geldige code.** Een apparaat waarop bij het openen een code
  staat, begint geen proef: dat gezin heeft al betaald, en als de code afloopt
  is dat een verlenging. Een build zonder premiumserver begint ook geen proef.
- **`usePremium().actief` is een code of de proef.** Elk slot vraagt alleen
  dat, dus de proef opent precies wat een code opent. `metCode` is er voor de
  plekken die over de code zelf gaan (de premiumpagina en de ouderpagina).
- **De voordeur zegt het drie keer, en daartussen niets:** de eerste drie dagen
  wat er open staat en voor hoe lang (zonder knop), de laatste drie dagen hoeveel
  dagen er nog zijn, en de week na afloop dat het voorbij is en dat er niets weg
  is. De laatste twee met de gewone weg naar premium.
- **De premiumpagina en de ouderpagina** noemen de laatste dag als datum, en na
  afloop dat alles nog op het apparaat staat.

### Afwegingen

- **Veertien dagen, niet zeven.** Een kind moet een diploma kunnen halen en een
  week met weekdoelen doormaken, en een ouder moet "Hoe gaat het?" met echte
  cijfers zien. Dat past niet in een week waarin ook een weekend valt.
- **Een hek, geen kluis.** Wie de opslag van de browser wist, krijgt de proef
  opnieuw. Voor deze doelgroep weegt dat niet op tegen een proef die zonder
  account en zonder server werkt. Met het gezinsaccount kan de proef later per
  gezin op de server worden bijgehouden.
- **Omgekeerde proef in plaats van een proef op aanvraag.** Wie eerst alles
  heeft, mist het daarna concreet; wie eerst om een proef moet vragen, begint
  er vaak niet aan. Of het hier werkt, is een hypothese tot de verkopen het
  laten zien.

### Gevolgen

- De e2e-tests die sloten nakijken, beginnen met `ZONDER_CODE`
  (`e2e/zonderCode.ts`): geen code en een proef uit 2000. Een lege opslag is
  sinds deze wijziging een apparaat met twee weken premium.
- `e2e/proef.spec.ts` loopt het begin, het aftellen, het einde en de stilte
  daarna na, en dat een apparaat met een code geen proef begint.
